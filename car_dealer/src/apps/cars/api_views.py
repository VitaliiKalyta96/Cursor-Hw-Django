...
from src.apps.cars.permissions import CustomAuthentication
from src.apps.cars.serializers import CarListSerializer, CarCreateUpdateSerializer, \
    ColorSerializer, BrandSerializer, \
...
class ApiCarPublicListView(viewsets.ViewSet, PageNumberPagination):
    ...
    def detail(self, request, car_pk):
...
class CRUDModelView(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Model.objects.filter(dealer__user=self.request.user)
